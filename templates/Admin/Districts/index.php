                    ...
                    <table id="<?php echo str_replace(' ', '', 'District'); ?>" class="table table-hover table-bordered table-striped">
                        <thead>
                            <tr>
                                ...
                                <th class="text-center"><?= $this->Paginator->sort('Districts.id', __d('setting', 'region')) ?></th>
                        ...
                        <tbody class="text-center">
                            <?php foreach ($districts as $district) : ?>
                                <tr>
                                    ...
                                    <td>
                                        <?= $district->has('region') ? $this->Html->link(
                                            h($district->region->region_languages[0]->name),
                                            ['controller' => 'Regions', 'action' => 'view', $district->region->id]
                                        ) : '' ?>
                                    </td>
                                    <td>
                                        <?=  $this->Html->link(
                                            h($district->DistrictLanguages['name']),
                                            ['controller' => 'Districts', 'action' => 'view', $district->id]
                                        )  ?>
                                    </td>
                                    <td class="text-center">
                                        <?= $this->element('view_check_ico', array('_check' => $district->enabled)) ?>
                                    </td>
                                    <td class="text-center"><?= h($district->created) ?></td>
                                    <td class="text-center"><?= h($district->modified) ?></td>
                                    <td class="text-center">
